...
export default function Rules() {
  ...
  // Score de spécificité d'une règle
  ...
  function describeRule(rule) {
    ...
    if (rule.year_from || rule.year_to) {
      parts.push(`${rule.year_from || '?'}–${rule.year_to || '?'}`)
    }
            ...
            <table>
              ...
              <tbody>
                {filtered.map(rule => (
                  <tr key={rule.id}>
                    <td>
                      <div style={{ fontWeight: 600, color: 'var(--gray-900)' }}>{rule.maintenance_types?.code}</div>
                      <div style={{ fontSize: 12, color: 'var(--gray-500)' }}>{rule.maintenance_types?.name}</div>
                    </td>
                    <td>
                      <span className="badge badge-gray">{describeRule(rule)}</span>
                    </td>
                    <td>
                      {rule.initial_months || rule.initial_km ? (
                        <span className="mono">
                          {rule.initial_months ? `${rule.initial_months}m` : '—'}
                          {' / '}
                          {rule.initial_km ? `${rule.initial_km.toLocaleString()}km` : '—'}
                        </span>
                    ...
                    <td>
                      {rule.repeat_months || rule.repeat_km ? (
                        <span className="mono">
                          {rule.repeat_months ? `${rule.repeat_months}m` : '—'}
                          {' / '}
                          {rule.repeat_km ? `${rule.repeat_km.toLocaleString()}km` : '—'}
                        </span>
                      ) : <span style={{ color: 'var(--gray-300)' }}>—</span>}
                    </td>
                    <td>
                      {rule.price ? (
                        <span style={{ fontWeight: 600 }}>{parseFloat(rule.price).toFixed(2)} $</span>
                      ) : <span style={{ color: 'var(--gray-300)' }}>—</span>}
                    </td>
                    <td>
                      <span className="score-pill" title="Score de spécificité">{getScore(rule)}</span>
                    </td>
                    <td>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              {/* Critères */}
              <div className="section-label" style={{ marginTop: 20 }}>Critères de ciblage <span style={{ fontWeight: 400, textTransform: 'none', fontSize: 11 }}>(laisser vide = s'applique à tous)</span></div>

              <div className="form-row form-row-2" style={{ marginBottom: 12 }}>
                <div>
                  <label>Année de</label>
                  <input type="number" placeholder="ex: 2015" value={form.year_from} onChange={e => setField('year_from', e.target.value)} />
                </div>
                <div>
                  <label>Année à</label>
                  <input type="number" placeholder="ex: 2023" value={form.year_to} onChange={e => setField('year_to', e.target.value)} />
                </div>
              </div>
              ...
              <div className="form-row form-row-2" style={{ marginBottom: 12 }}>
                <div>
                  <label>Marque</label>
                  <input placeholder="ex: Ford, Subaru..." value={form.make} onChange={e => setField('make', e.target.value)} />
                </div>
                <div>
                  <label>Modèle</label>
                  <input placeholder="ex: F-150, Crosstrek..." value={form.model} onChange={e => setField('model', e.target.value)} />
                </div>
              </div>
              ...
              <div className="form-group">
                <label>Moteur</label>
                <input placeholder="ex: 2.0L, V8... (optionnel)" value={form.engine} onChange={e => setField('engine', e.target.value)} />
              </div>
